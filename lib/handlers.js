...
exports.movies = (req, res) => {
  const fetchMovies = async () => {
    ...
    if (cachedResults) {
      ...
      const tredning = await tmdb.tmdbRequest.get("/trending/movie/day");
      results = { popular: data.results, trending: tredning.data.results };
...
exports.getMovieList = async (req, res) => {
  ...
  try {
    let results;
    let myList = "listCached";
    let isCached = false;
    const cachedResults = await redisClient.get(myList);
    const parseCacheResult = JSON.parse(cachedResults);
    ...
    if (cachedResults && parseCacheResult.database.length == movieList.length) {
      isCached = true;
      results = parseCacheResult;
    } else {
      ...
      await redisClient.set(myList, JSON.stringify(results), {
        EX: 7200,
        NX: true,
      });
    }

    return res.status(200).json({ fromCache: isCached, data: results });
  } catch (error) {
