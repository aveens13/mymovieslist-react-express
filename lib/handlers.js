...
exports.searchMovie = async (req, res) => {
  ...
  const resultsFromDatabase = await prisma.user.findMany({
    where: {
      name: {
  ...
  let returnData = [...data.results, ...resultsFromDatabase];
...
exports.hasFollowRelation = async (req, res) => {
  const { userID, targetUserID } = req.body;
  try {
    const followingList = await get_followings(userID);
    if (followingList.includes(targetUserID)) {
    ...
    console.log(error);
    return res.status(400).send("Cannot find the relation");
  }
};
